...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    model_input_names: List[str] = ["input_ids", "token_type_ids", "attention_mask", "ext_labels"]
    ...
    def pad(
            ...
            max_length: Optional[int] = None,
            max_length_ext_labels: Optional[int] = None,
            pad_to_multiple_of: Optional[int] = None,
            return_attention_mask: Optional[bool] = None,
            return_tensors: Optional[Union[str, TensorType]] = None,
            verbose: bool = True,
        ...
        # if not target_side and 'ext_labels' in self.model_input_names:
        if  'ext_labels' in self.model_input_names and 'ext_labels' in encoded_inputs.keys():
            required_input_ext_labels = encoded_inputs[self.model_input_names[-1]]
        ...
        if padding_strategy == PaddingStrategy.LONGEST:
            ...
            # if not target_side:
            if 'ext_labels' in self.model_input_names and 'ext_labels' in encoded_inputs.keys():
                max_length_ext_labels = max(len(inputs) for inputs in required_input_ext_labels)
            padding_strategy = PaddingStrategy.MAX_LENGTH
        try:
            batch_outputs = {}
            for i in range(batch_size):
                ...
                outputs = self._pad(
                    inputs,
                    max_length=max_length,
                    max_length_ext_labels=max_length_ext_labels,
                    padding_strategy=padding_strategy,
                    pad_to_multiple_of=pad_to_multiple_of,
                    return_attention_mask=return_attention_mask,
                    training=True
            ...
            be = BatchEncoding(batch_outputs, tensor_type=return_tensors)
        except:
            import pdb;pdb.set_trace()
        return be
    ...
    def prepare_for_model(
            ...
            verbose: bool = True,
            prepend_batch_axis: bool = False,
            ext_labels: Optional[List[int]] = None,
            target_side: bool = False,
            **kwargs
    ) -> BatchEncoding:
        """
        ...
        try:
            if ext_labels is not None:
                encoded_inputs['bos_ids'] = [i for i, v_id in enumerate(sequence) if v_id == 2]

                encoded_inputs["ext_labels"] = ext_labels[:len(encoded_inputs['bos_ids'])]

                # if len(encoded_inputs['bos_ids']) != len(ext_labels[:len(encoded_inputs['bos_ids'])]):
                #     import pdb;pdb.set_trace()
                assert len(encoded_inputs['bos_ids']) == len(encoded_inputs["ext_labels"]), 'Preparing for model: ext_labels` and `bos_ids` need to be of the same size'
        except:
            encoded_inputs["ext_labels"] = encoded_inputs["ext_labels"] + [0] * (len(encoded_inputs['bos_ids']) - len(encoded_inputs["ext_labels"]))
            print('`ext_labels` extended!')
            # import pdb;pdb.set_trace()
    ...
    def _pad(
        ...
        required_input = encoded_inputs[self.model_input_names[0]]
        if max_length_ext_labels is not None and "ext_labels" in self.model_input_names:
            required_input_ext_labels = encoded_inputs[self.model_input_names[-1]]
            required_input_bos_ids = encoded_inputs[self.model_input_names[-2]]

            # assert len(required_input_ext_labels) == len(required_input_bos_ids),  print('`Padding: ext_labels` and `bos_ids` need to be of the same size')

        if padding_strategy == PaddingStrategy.LONGEST:
            max_length = len(required_input)
            if max_length_ext_labels is not None and "ext_labels" in self.model_input_names:
                max_length_ext_labels = len(required_input_ext_labels)

        if max_length is not None and pad_to_multiple_of is not None and (max_length % pad_to_multiple_of != 0):
            max_length = ((max_length // pad_to_multiple_of) + 1) * pad_to_multiple_of

        needs_to_be_padded = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(required_input) != max_length
        needs_to_be_padded_ext_labels = False
        if max_length_ext_labels is not None and "ext_labels" in self.model_input_names:
            needs_to_be_padded_ext_labels = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(
            required_input_ext_labels) != max_length_ext_labels
        ...
        if needs_to_be_padded_ext_labels:
            if self.padding_side == "right":
                difference_ext_labels = max_length_ext_labels - len(required_input_ext_labels)
                # print(f'{max_length_ext_labels} / {difference_ext_labels}' )
                encoded_inputs[self.model_input_names[-1]] = required_input_ext_labels + [
                    -100] * difference_ext_labels
                encoded_inputs[self.model_input_names[-2]] = required_input_bos_ids + [0] * difference_ext_labels
