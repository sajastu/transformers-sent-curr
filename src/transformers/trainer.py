...
from multiSumm.model.neural import MultiTaskLoss
...
class Trainer:
    ...
    def __init__(
        ...
        optimizers: Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR,
                          torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR] = (None, None, None, None),
    ):
        if args is None:
        ...
        self.optimizer, self.lr_scheduler, self.optimizer_ext, self.lr_scheduler_ext = optimizers
        ...
        self.callback_handler = CallbackHandler(
            callbacks, self.model, self.tokenizer, self.optimizer, self.lr_scheduler, self.optimizer_ext, self.lr_scheduler_ext
        )
        ...
        if args.fp16 and not args.deepspeed:  # deepspeed manages its own fp16
            if self.fp16_backend == "amp":
                ...
                if is_sagemaker_mp_enabled():
                    ...
                    if self.use_second_opt:

                        self.scaler_ext = torch.cuda.amp.GradScaler()
            else:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        if self._signature_columns is None:
            ...
            self._signature_columns += ["label", "label_ids", "ext_labels", "bos_ids"]
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.use_second_opt:
                decay_parameters = [name for name in decay_parameters if "bias" not in name and not ('_sent' in name or name.startswith('model.encoder'))]
            else:
        ...
        if self.optimizer_ext is None and self.use_second_opt:
            decay_parameters = get_parameter_names(self.model, [nn.LayerNorm])
            decay_parameters = [name for name in decay_parameters if "bias" not in name]
            decay_parameters = [name for name in decay_parameters if "bias" not in name and ('_sent' in name or name.startswith('model.encoder'))]

            optimizer_grouped_parameters_ext = [
                {
                    "params": [p for n, p in self.model.named_parameters() if n in decay_parameters],
                    "weight_decay": self.args.weight_decay,
                },
            ...
            if self.sharded_ddp == ShardedDDPOption.SIMPLE:
                self.optimizer_ext = OSS(
                    params=optimizer_grouped_parameters_ext,
                    optim=optimizer_cls,
                    **optimizer_kwargs,
                )
            else:
                self.optimizer_ext = optimizer_cls(optimizer_grouped_parameters_ext, **optimizer_kwargs)

        if is_sagemaker_mp_enabled():
            self.optimizer_ext = smp.DistributedOptimizer(self.optimizer_ext)
    ...
    def create_scheduler(self, num_training_steps: int):
        ...
        if self.lr_scheduler_ext is None and self.use_second_opt:
            warmup_steps_ext = (
                5000
                if self.args.warmup_steps > 0
                else math.ceil(num_training_steps * self.args.warmup_ratio)
            ...
            self.lr_scheduler_ext = get_scheduler(
                self.args.lr_scheduler_type,
                self.optimizer_ext,
                num_warmup_steps=warmup_steps_ext,
                num_training_steps=num_training_steps,
            )
    ...
    def train(
        ...
        **kwargs,
        ...
        if self.model_init is not None:
            ...
            if self.use_second_opt:
                self.optimizer_ext, self.lr_scheduler_ext = None, None
        ...
        if self.use_second_opt:
            self.callback_handler.optimizer_ext = self.optimizer_ext
            self.callback_handler.lr_scheduler_ext = self.lr_scheduler_ext
        ...
        # tr_loss_abs = torch.tensor(0.0).to(args.device)
        # tr_loss_ext = torch.tensor(0.0).to(args.device)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed:
                        ...
                        if self.use_amp:
                            ...
                            if self.use_second_opt:
                                self.scaler_ext.unscale_(self.optimizer_ext)

                        if hasattr(self.optimizer, "clip_grad_norm"):
                            # Some optimizers (like the sharded optimizer) have a specific way to do gradient clipping
                            self.optimizer.clip_grad_norm(args.max_grad_norm)
                            if self.use_second_opt:
                                self.optimizer_ext.clip_grad_norm(args.max_grad_norm)
                            ...
                            nn.utils.clip_grad_norm_(
                                amp.master_params(self.optimizer) if self.use_apex else [p for n, p in
                            ...
                            if self.use_second_opt:
                                nn.utils.clip_grad_norm_(
                                    amp.master_params(self.optimizer) if self.use_apex else [p for n, p in
                                                                                             ...
                                                                                             not (
                                                                                                         '_sent' in n or n.startswith(
                                                                                                     'model.encoder'))],
                                    args.max_grad_norm,
                                )
                                nn.utils.clip_grad_norm_(
                                    amp.master_params(self.optimizer_ext) if self.use_apex else [p for n, p in model.named_parameters() if ('_sent' in n or n.startswith('model.encoder'))],
                                    args.max_grad_norm,
                                )
                    ...
                    if self.deepspeed:
                        ...
                        self.scaler.update()
                        if self.use_second_opt:
                            self.scaler_ext.step(self.optimizer_ext)
                            self.scaler_ext.update()
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        if self.control.should_log:
            ...
            # model.super_loss.tau = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            ...
            if self.use_second_opt:
                logs["learning_rate_ext"] = self.lr_scheduler_ext.get_last_lr()[0]
